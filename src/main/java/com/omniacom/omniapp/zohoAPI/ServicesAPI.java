...
public class ServicesAPI {
	...
	public Tasklist pushTaskList(Service service, String projectId, User user) throws IOException {
		taskListsApi = new TasklistsAPI(usersApi.getUserAuthToken(user), usersApi.getPortalId(user));
	...
	public List<Service> pushAllTaskLists() {
		...
		for (Operation op : syncedOperations) {
			...
			System.out.println("TaskLists to sync in Milestone '" + op.getName() + "': " + unsyncedServices.size());

			for (Service service : unsyncedServices) {
				try {
					// System.out.println("project zoho id:
					// "+String.valueOf(op.getProject().getZohoId()));
