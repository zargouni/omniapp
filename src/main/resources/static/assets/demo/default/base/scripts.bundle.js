...
var mApp = function() {
    ...
    return {
		 ...
		 *            el jQuery element object
		 * @param {number}
		 *            offset Offset to element scroll position
		 */
		 ...
		 *            el jQuery element object
		 * @param {object}
		 *            options mCustomScrollbar plugin options(refer:
        ...
        /**
		 * Blocks element with loading indiciator using
		 * http://malsup.com/jquery/block/
		 * 
		 * @param {object}
		 *            target jQuery element object
		 * @param {object}
		 *            options
		 */
...
var mUtil = function() {
    ...
    /**
	 * Handle window resize event with some delay to attach event handlers upon
	 * resize complete
	 */
    ...
    return {
		 ...
		 * 
		 * @param {string}
		 *            mode Responsive mode name(e.g: desktop,
		 ...
		 * 
		 * @param {string}
		 *            mode Responsive mode name(e.g: xl, lg, md, sm)
		 * @returns {number}
		 */
		 ...
		 * 
		 * @param {object}
		 *            obj Object contains values paired with given key path
		 * @param {string}
		 *            keys Keys path seperated with dots
		 * @returns {object}
		 */
...
(function($) {	if (typeof mUtil === 'undefined') throw new Error('mUtil is required and must be included before mDatatable.');	// plugin setup
	$.fn.mDatatable = function(options) {		if ($(this).length === 0) throw new Error('No mDatatable element exist.');		// global variables
		...
		datatable.debug = false;		datatable.API = {			record: null,			value: null,			params: null,		};		var Plugin = {			/*******************************************************************
			 ...
			 * Setup extra system column properties Note: selector checkbox,
			 * subtable toggle
			 ...
			 */			dataRender: function(action) {				$(datatable.table).					siblings('.m-datatable__pager').					removeClass('m-datatable--paging-loaded');				var buildMeta = function() {					datatable.dataSet = datatable.dataSet || [];					Plugin.localDataUpdate();					// local pagination meta
					...
					if (options.pagination) {						if (options.data.serverPaging && options.data.type !== 'local') {							// server pagination
							var serverMeta = Plugin.getObject('meta', result || null);							if (serverMeta !== null) {								Plugin.paging(serverMeta);							} else {								// no meta object from server response, fallback
								// to local pagination
				...
				if (options.data.type === 'local'					// for remote json datasource
					|| typeof options.data.source.read === 'undefined' &&					datatable.dataSet !== null					// for remote datasource, server sorting is disabled and
					// data already received from remote
			 ...
			 */			paging: function(meta, callback) {				var pg = {					meta: null,					pager: null,					paginateEvent: null,					pagerLayout: {pagination: null, info: null},					callback: null,					init: function(meta) {						pg.meta = meta;						// todo; if meta object not exist will cause error
						...
						$(datatable).ready(function() {							$('.selectpicker').								selectpicker().								siblings('.dropdown-toggle').								attr('title', Plugin.getOption(									'translate.toolbar.pagination.items.default.select'));						});						pg.paste();					},					paste: function() {						// insert pagination based on placement position,
						// top|bottom
						...
						$(pg.pager).trigger('m-datatable--on-goto-page', pg.meta);					},					updatePerpage: function(e) {						e.preventDefault();						if (Plugin.getOption('layout.height') === null) {							// fix white space, when perpage is set from many
							// records to less records
			 ...
			 */			setupSubDatatable: function() {				var subTableCallback = Plugin.getOption('detail.content');				if (typeof subTableCallback !== 'function') return;				// subtable already exist
				...
				var toggleSubTable = function(e) {					e.preventDefault();					// get parent row of this subtable
					...
					var subTableRow = $(parentRow).next('.m-datatable__row-subtable');					if ($(subTableRow).length === 0) {						// prepare DOM for sub table, each <tr> as parent and
						// add <tr> as child table
			 ...
			 * Insert html into table content, take count mCustomScrollbar DOM
			 * to prevent replace
			 * 
			 * @param html
			 * @param tablePart
			 ...
			 * Get table unique ID Note: table unique change each time refreshed
			 * 
			 * @param suffix
			 * @returns {*}
			 ...
			 * Sort table row at HTML level by column index. todo; Not in use.
			 * 
			 * @param header
			 *            Header sort clicked
			 * @param sort
			 ...
			 * @param int
			 *            Boolean. Optional. Comparison value parse to integer.
			 *            Default false
			 ...
			 * 
			 * @param path
			 *            String Dot notation path in string
			 * @param object
			 *            Object to iterate
			 * @returns {*}
			 ...
			 * @deprecated in v5.0.6 Get column of current record ID
			 * @param columnName
			 * @returns {jQuery}
			 ...
			 * @deprecated in v5.0.6 Get current selected column value
			 * @returns {jQuery}
			 */			getValue: function() {				return $(datatable.API.value).text();			},			/**
			 * Set checkbox active
			 * 
			 * @param cell
			 *            JQuery selector or checkbox ID
			 ...
			 * @deprecated in v5.0.6 Get selected rows which are active
			 * @returns {jQuery}
			 ...
			 * @param value
			 * @param columns.
			 *            Optional list of columns to be filtered.
			 ...
			 * @deprecated in v5.0.6 Hide column by column's field name
			 * @param fieldName
			 */			hideColumn: function(fieldName) {				// add hide option for this column
				$.map(options.columns, function(column) {					if (fieldName === column.field) {						column.responsive = {hidden: 'xl'};					}					return column;				});				// hide current displayed column
				var tds = $.grep($(datatable.table).find('.m-datatable__cell'), function(n, i) {					return fieldName === $(n).data('field');				});				$(tds).hide();			},			/**
			 * @deprecated in v5.0.6 Show column by column's field name
			 * @param fieldName
	...
	$.fn.mDatatable.defaults = {		// datasource definition
		data: {			type: 'local',			source: null,			pageSize: 10, // display records per page
			saveState: {				// save datatable state(pagination, filtering, sorting, etc) in
				// cookie or browser webstorage
				cookie: false,				webstorage: true,			},			serverPaging: false,			serverFiltering: false,			serverSorting: false,			autoColumns: false,		},		// layout definition
		layout: {			theme: 'default', // datatable will support multiple themes and
								// designs
			class: 'm-datatable--brand', // custom wrapper class
			scroll: false, // enable/disable datatable scroll both horizontal
							// and vertical when needed.
		...
		columns: [],		search: {			// enable trigger search by keyup enter
			...
			autoHide: false,		},		// toolbar
		toolbar: {			// place pagination and displayInfo blocks according to the array
			// order
			layout: ['pagination', 'info'],			// toolbar placement can be at top or bottom or both top and bottom
			// repeated
			...
			items: {				// pagination
				pagination: {					// pagination type(default or scroll)
					...
					pageSizeSelect: [] // display dropdown to select pagination
										// size. -1 is used for "ALl" option
				},				// records info
				info: true,			},		},		// here we will keep all strings and message used by datatable UI so
		// developer can easiliy translate to any language.
		// By default the stirngs will be in the plugin source and here can
		// override it
            ...
            toggle: function() {
                ...
                $(window).scroll(function() {
                    ...
                    if (
                        ...
                        if (st > offset && lastScrollTop < st){ // down scroll
																// mode
                            $("body").addClass(on);
                            $("body").removeClass(off);
                        } else { // back scroll mode
...
(function($) {
        ...
        var Plugin = {
            ...
            resize: function(e) {
                ...
                if (
                    ...
                    var moved = submenu.find('> .m-menu__subnav > .m-menu__item').length; // currently
																							// move
                    var left = element.find('> .m-menu__nav > .m-menu__item:not(.m-menu__item--resize)').length; // currently
																													// left
    ...
    $.fn.mMenu.defaults = {
        ...
        accordion: {   
            ...
            autoScroll: true, // enable auto scrolling(focus) to the clicked
								// menu item
            expandAll: true   // allow having multiple expanded accordions in
								// the menu
        },
        
        // dropdown submenu mode
        dropdown: {
            timeout: 500  // timeout in milliseconds to show and hide the
							// hoverable submenu dropdown
        }
    }; 
            ...
            build: function() {
                // == Next button event handler
            	 ...
            	 wizard.btnNext.on('click', function (e) { 
            		 // make ajax call to update added client to the backing bean
 					// no publish to db
            		 if ($('#m_wizard_form_step_1').hasClass("m-wizard__form-step--current")){
                     ...
                     // grab sites that are added manually and update to backing
    					// bean
                    	if ($('#m_wizard_form_step_2').hasClass("m-wizard__form-step--current")){
                    		if(wizardSaveManualAddedSites()){
                    				populateConfirmationStepWizard();
                    				e.preventDefault();
                       				Plugin.goNext();
                    		}
                    	}
...
(function($) {
	...
	/**
	 * @param datatable
	 *            Main datatable plugin instance
	 * @param options
	 *            Extension options
	 * @returns {*}
	 */
		...
		var Extension = {
			...
			initEvent: function() {
				...
				$(datatable.tableBody).find('.m-checkbox--single > [type="checkbox"]').click(function(e) {
					...
					if (!options.vars.requestIds && Extension.selectedRows.length < 1) {
						// remove select all checkbox, if there is no checked
						// checkbox left
			...
			initSelect: function() {
				...
				if (Extension.selectedAllRows && options.vars.requestIds) {
					...
					// local checkbox; check if all checkboxes of currect page
					// are checked
			...
			getSelectedId: function(path) {
				...
				if (Extension.selectedAllRows && options.vars.requestIds) {
					...
					if (selectedAllRows.length > 0) {
						// remove single unselected rows from selectedAllRows
						// ids from server response emta
						Extension.unselectedRows.forEach(function(id) {
...
jQuery.validator.setDefaults({
    ...
    unhighlight: function(element) { // revert the change done by hightlight
        ...
        group.removeClass('has-danger'); // set error class to the control
											// group
    },
    ...
    success: function(label, element) {
        ...
        // group.addClass('has-success').removeClass('has-danger'); // set
		// success class and hide error class
 ...
 // handle horizontal menu
        ...
        var menuOptions = {  
            ...
            submenu: {
                desktop: {
                    ...
                    // whenever body has this class switch the menu mode to
					// dropdown
                    state: {
    ...
    var initLeftAsideToggle = function() {
        var asideLeftToggle = $('#m_aside_left_minimize_toggle').mToggle({
            ...
            // server side code and add "m-brand--minimize
			// m-aside-left--minimize" classes to
            // the body tag in order to initialize the minimized left asode mode
			// during page loading.
        });
...
var mQuickSidebarAdd = function() {
    var topbarAside = $('#m_quick_sidebar_add');
    var topbarAsideTabs = $('#m_quick_sidebar_add_tabs');    
    var topbarAsideClose = $('#m_quick_sidebar_add_close');
    var topbarAsideToggle = $('#m_quick_sidebar_add_toggle');
    var topbarAsideContent = topbarAside.find('.m-quick-sidebar-add__content');

    var initMessages = function() {
        var messenger = $('#m_quick_sidebar_add_tabs_new_task');  
       ...
       /*
		 ...
		 */
      /*
		 * var init = function() { var height = topbarAside.outerHeight(true) -
		 * topbarAsideTabs.outerHeight(true) -
		 * messenger.find('.m-messenger__form').outerHeight(true) - 120; // init
		 * messages scrollable content messengerMessages.css('height', height);
    ...
    var initSettings = function() { 
        var settings = $('#m_quick_sidebar_add_tabs_new_service');
    ...
    var initLogs = function() {
        // init dropdown tabbable content
        var logs = $('#m_quick_sidebar_add_tabs_new_operation');
...
var mNewProjectSidebar = function() {
    var topbarAside = $('.m_new_project_sidebar');
    // var topbarAsideTabs = $('#m_new_project_sidebar_tabs');
    var topbarAsideClose = $('#m_new_project_sidebar_close');
    var topbarAsideToggle = $('#m_new_project_sidebar_toggle');
...
var mNewProjectSelectZone = function() {
    ...
    switchZoneWizard.change(function() {
        if(this.checked) {
        	zoneBlockWizard.show();        }
        else{
        	zoneBlockWizard.hide();
        }
    });
...
var mOperationEditSidebar = function() {
    var topbarAside = $('.m_operation_edit_sidebar');
    // var topbarAsideTabs = $('#m_project_details_sidebar_tabs');
    var topbarAsideClose = $('#m_operation_edit_sidebar_close');
    var topbarAsideToggle = $('#m_operation_edit_sidebar_toggle');
    var topbarAsideContent = topbarAside.find('.m-operation-edit-sidebar__content');

    var initForm = function() {
        var messenger = $('#m_operation_edit_sidebar_tabs_new_task');  
    ...
    var initOffcanvas = function() {
        topbarAside.mOffcanvas({
            class: 'm-operation-edit-sidebar',
            overlay: true,  
            close: topbarAsideClose,
            toggle: topbarAsideToggle
        });   
